 ...
 * params (incl. colors) smoothly, so switching states IS the animation.
 * "Zuhören" reacts to the microphone, "Sprechen" to the demo voice audio
 * (Web Audio AnalyserNode → RMS drives scale/distortion on top of the preset).
 */
...
const STATES: Record<string, { label: string; hint: string; p: Params }> = {
  idle: {
    label: 'Ruhe',
    hint: 'Langsame Atmung — Grundzustand in der App.',
  ...
  listening: {
    label: 'Zuhören',
    hint: 'Reagiert auf DEINE Stimme (Mikrofon einschalten!). Cyan-lastig, puls-sensibel.',
  ...
  thinking: {
    label: 'Nachdenken',
    hint: 'Swirl zirkuliert nach innen, Gedankenrauschen über Grain.',
  ...
  speaking: {
    label: 'Sprechen',
    hint: 'Pulsiert zur eigenen Stimme (spielt Demo-Audio), Mint blitzt bei Betonungen.',
  ...
  success: {
    label: 'Verstanden ✓',
    hint: 'Einmaliger Mint-Puls, kehrt danach selbst zur Ruhe zurück.',
  ...
  unsure: {
    label: 'Unsicher',
    hint: 'Wobbelt langsam — wie ein schief gelegter Kopf vor einer Rückfrage.',
  ...
  transfer: {
    label: 'Durchstellen',
    hint: 'Der Orb reist zur Seite — er bringt den Anruf woanders hin.',
  ...
  offline: {
    label: 'Offline',
    hint: 'Entsättigt, fast eingefroren.',
...
export default function StatesPage() {
  const [stateKey, setStateKey] = useState<keyof typeof STATES>('idle')
  const [micOn, setMicOn] = useState(false)
  const [level, setLevel] = useState(0)
  const [rendered, setRendered] = useState<Params>(STATES.idle.p)

  const current = useRef<Params>({ ...STATES.idle.p, colors: [...STATES.idle.p.colors] })
  const target = useRef<Params>(STATES.idle.p)
  ...
  // state switching + one-shot success + speaking audio
  useEffect(() => {
    target.current = STATES[stateKey].p
    if (stateKey === 'success') {
  ...
  const toggleMic = async () => {
    ...
    try {
      ...
      alert('Mikrofon-Zugriff abgelehnt.')
    }
  }
  ...
  // animation loop: lerp current → target, add audio modulation, ~30fps render
  useEffect(() => {
    let raf = 0
    let lastRender = 0
    ...
    const tick = (now: number) => {
      ...
      // breathing on idle, audio pulse on listening/speaking
      ...
      <div className="orb-stage">
        <div className="orb" style={{ transform: `translateX(${rendered.offsetX * 260}px) scale(${rendered.scale})` }}>
          <MeshGradient
            ...
            distortion={rendered.distortion}
            swirl={rendered.swirl}
            grainMixer={rendered.grainMixer}
            grainOverlay={0}
          />
      ...
      <aside className="controls">
        ...
        <div className="control">
          ...
          <div className="seg states-grid">
            {(Object.keys(STATES) as (keyof typeof STATES)[]).map((k) => (
              <button
                key={k}
                className={stateKey === k ? 'seg-btn active' : 'seg-btn'}
                onClick={() => (k === 'speaking' ? speakDemo() : setStateKey(k))}
              >
                {STATES[k].label}
              </button>
            ))}
        ...
        <div className="control">
          ...
          <div className="seg">
            ...
            <button className="seg-btn" onClick={speakDemo}>Lena spricht (Demo)</button>
          </div>
        ...
        <p className="control-hint">
          Jeder Zustand ist nur ein Parameter-Set — der Übergang wird live
          interpoliert. Mit Mikrofon an reagiert „Zuhören" auf deine Stimme,
          „Sprechen" pulsiert zum Demo-Audio.
        </p>
      </aside>
