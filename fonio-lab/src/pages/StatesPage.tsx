...
export default function StatesPage() {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      const k = 0.07
      // 1. Zustandswechsel: weich auf die Zielwerte zulaufen
      ...
      // 2. Stimme obendrauf — dieselbe Hüllkurve und dieselben Zuschläge wie in
      // der Engine, damit Preview, Export-Vorschau und Render übereinstimmen.
      const audible = stateRef.current === 'listening' || stateRef.current === 'speaking'
      const dt = Math.min(0.1, (now - lastRef.current) / 1000)
      lastRef.current = now
      envRef.current = envStep(envRef.current, audible ? lvl : 0, DEFAULT_VOICE, dt)
      ...
      if (now - lastRender > 33) {
        ...
        setRendered({
          ...c,
          speed: c.speed + env * V.speed,
          distortion: Math.max(0, c.distortion + env * V.distortion),
          swirl: Math.max(0, c.swirl + env * V.swirl),
          scale: Math.max(0.1, c.scale * (1 + env * V.scale) + breath),
          soften: Math.max(0, Math.min(1, c.soften + env * V.soften)),
          colors: [...c.colors],
        })
        setLevel(lvl)
      }
